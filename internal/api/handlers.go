...
func HandleChatCompletions(proxyClient *proxy.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if targetModel.ID == 0 || len(targetModel.Backends) == 0 {
			c.AbortWithStatusJSON(http.StatusNotFound, gin.H{
				"error": gin.H{
					"message": "所请求的模型未配置或当前无可用后端实例",
		...
		// 2. 筛选支持 chat 协议的健康实例
		var candidate *models.Backend
		for i := range targetModel.Backends {
			b := &targetModel.Backends[i]
			if b.IsHealthy && b.SupportsProtocol(models.ProtocolChat) {
				candidate = b
				break
			}
		}
		...
		if candidate == nil {
			c.AbortWithStatusJSON(http.StatusServiceUnavailable, gin.H{
				"error": gin.H{
					"message": "当前模型所有可用实例均处于离线或过载维护状态",
		...
		if !candidate.AcquireSlot() {
			c.AbortWithStatusJSON(http.StatusTooManyRequests, gin.H{
				"error": gin.H{
					"message": "后端物理算力节点并发已饱和，请稍后重试",
					"type":    "rate_limit_error",
				},
			})
			return
		}
		defer candidate.ReleaseSlot()

		// 4. 代理转发请求
		result, forwardErr := proxyClient.ForwardChatCompletions(c, candidate, bodyBytes, req.Stream)

		// 5. 异步落盘审计日志与 Credits 扣减
		go recordAuditAndDeductCredits(c.Copy(), targetModel, candidate, result, forwardErr)
	}
}
